...
async function loadCloud(){
 ...
 persistLocal();cloudAvailable=true;return true;
}

function render(){
 const existing=document.querySelector('#journalView');if(existing)existing.remove();
 const wrap=document.createElement('div');wrap.id='journalView';wrap.className='journal-backdrop';
 wrap.innerHTML=`<section class="journal-shell" role="dialog" aria-modal="true" aria-label="Coffee Journal"><header class="journal-head"><div><span class="eyebrow">BREW MEMORY</span><h1>Coffee Journal</h1><p>Keep the details that make the next cup better.</p></div><button class="close" id="journalClose">×</button></header><div class="journal-toolbar"><button class="primary" id="journalNew">+ New entry</button><span>${entries.length} ${entries.length===1?'entry':'entries'}</span></div><div class="journal-list">${entries.length?entries.map(entryCard).join(''):`<div class="journal-empty"><div>✦</div><h2>Nothing logged yet.</h2><p>Record a bean, the recipe you used, what you tasted and what you want to change next time.</p><button class="secondary" id="journalEmptyNew">Start your first entry</button></div>`}</div></section>`;
...
function openEditor(id,prefill={}){
 const existing=entries.find(x=>x.id===id)||{id:'',title:'',bean:'',roaster:'',origin:'',recipe:'',brew_method:'',dose:'',water:'',grind:'',temperature:'',tasting_notes:'',changes:'',rating:0,...prefill};
...
window.addEventListener('pocketbarista:brew-complete',event=>{
 const brew=event.detail||{};
 setTimeout(()=>{
  const shouldLog=window.confirm(`Brew complete${brew.name?` — ${brew.name}`:''}. Would you like to log this cup in Coffee Journal?`);
  if(!shouldLog)return;
  openJournal();
  setTimeout(()=>openEditor('',{title:brew.name||'Coffee log',recipe:brew.name||'',brew_method:brew.method||'',dose:brew.coffee_g??'',water:brew.water_g??'',grind:brew.grind||'',temperature:brew.temperature_c??'',}),50);
 },250);
});
